...
def get_next_available_ip(prefix_id):
    return nb.ipam.prefixes.get(prefix_id).available_ips.list()[0].address
...
def dhcp_reservation(mac, ip, hostname):
    return f"dhcp-host={mac},{hostname},{ip.split('/')[0]},12h"
...
def main():
    ...
    for interface in get_interfaces_with_tag(INTERFACES_TAG):
        if interface.count_ipaddresses > 0:
            ...
            ip = next(nb.ipam.ip_addresses.filter(interface_id=interface.id)).address
        else:
            ...
            nb.ipam.ip_addresses.create(address=ip, assigned_object_type="dcim.interface", assigned_object_id=interface.id,
                                        description=f"[auto-dhcp] {interface.device.name} ({interface.name})", tag=INTERFACES_TAG)
